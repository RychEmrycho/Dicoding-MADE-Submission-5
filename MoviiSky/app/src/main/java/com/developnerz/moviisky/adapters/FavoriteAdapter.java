...
public class FavoriteAdapter extends RecyclerView.Adapter<FavoriteAdapter.ViewHolder> {
    ...
    public FavoriteAdapter(OnClickItemListener onClickItemListener) {
        this.onClickItemListener = onClickItemListener;
    }

    public void setMoviesList(Cursor moviesList){
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        if (TextUtils.isEmpty(movie.getOverview())) {
            ...
            holder.tv_movie_description.setText(movie.getOverview());
        }

        if (TextUtils.isEmpty(movie.getReleaseDate())) {
            setFieldEmpty(holder.tv_movie_date);
        } else {
            holder.tv_movie_date.setText("Release Date: " + movie.getReleaseDate());
        }
    ...
    private Movie getItem(int position){
        if (!moviesList.moveToPosition(position)){
            throw new IllegalStateException("Position Invalid");
        }
        return new Movie(moviesList);
    }
